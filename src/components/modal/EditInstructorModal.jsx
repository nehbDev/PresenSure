...
const EditInstructorModal = ({ isOpen, onClose, instructor, onSave }) => {
  ...
  const [previewImage, setPreviewImage] = useState(instructor?.instructor_photo || "");
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setPreviewImage(instructor?.instructor_photo || "");
      setTimeout(() => {
        setAnimate(true);
      }, 0);
    } else {
    ...
    >
      ...
      >
        ...
        <div className="bg-[#A9B5DF] flex justify-between items-center px-5 py-3 border-b rounded-t-lg border-gray-200">
          <h2 className="text-base font-bold text-[#2D336B]">Edit Instructor</h2>
          <button
            onClick={onClose}
