...
const dashboardData = {
  totalStudents: 120,
  totalSchedules: 15, // Replaced totalInstructors with totalSchedules
  pendingAppealsCount: 3, // Replaced activeSchedules with pendingAppealsCount
  recentAttendance: [
    { id: 1, class: "CS101", date: "2025-05-11", present: 25, absent: 5 },
    { id: 2, class: "CS102", date: "2025-05-11", present: 30, absent: 2 },
  ],
  pendingAppeals: [
    { id: 1, student: "John Doe", class: "CS101", date: "2025-05-10", description: "My attendance wasn’t marked for Advanced Mathematics class on Monday despite being present the full session.", status: "Pending" },
    { id: 2, student: "Jane Smith", class: "CS102", date: "2025-05-11", description: "Unable to access the online assessment platform during the scheduled quiz time.", status: "Approved" },
  ],
};

// Reusable Card Component with dynamic border color
const Card = ({ title, value, borderColor }) => (
  <div className={`bg-white p-4 rounded-lg shadow-md border-t-2 ${borderColor}`}>
  ...
  return (
    <div className="space-y-3 w-full max-w-[1800px] mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <Card title="Total Students" value={dashboardData.totalStudents} borderColor="border-green-500" />
        <Card title="Total Schedules" value={dashboardData.totalSchedules} borderColor="border-blue-500" />
        <Card title="Pending Appeals" value={dashboardData.pendingAppealsCount} borderColor="border-yellow-500" />
      </div>
